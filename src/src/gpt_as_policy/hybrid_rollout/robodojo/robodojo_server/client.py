...
class RoboDojoTools:
    ...
    def resume(self, checkpoint):
        ...
        self.tick = int(data["step_id"])
        self.history = json.loads(Path(data["history_path"]).read_text())
        if len(self.history) != int(data["history_length"]):
            raise RuntimeError("Checkpoint history mismatch")
        self.counters.update(data["counters"])
        self.run = json.loads((self.output/"run.json").read_text())
        self.require_native_termination = bool(
            self.run.get("require_native_termination")
        )
        self.source = (
            self.history[-1].get("source", "student")
            if self.history else "student"
        )
        i = len(self.history)
        obsdir = self.output/"observations"/f"{i:03d}"
        self.observation_path = obsdir/"observation.json"
        self.observation_packet = json.loads(
            self.observation_path.read_text()
        )
        with np.load(obsdir/"observation.npz") as z:
            self.obs = {
                k: z[k].copy()
                for k in self.observation_keys
            }
        req = self.output/f"request_{i-1:03d}.json"
        self.preceding = (
            json.loads(req.read_text())
            if i and req.exists() else None
        )
        self.phase = "infer"
        self._rpc("teacher_observation")
        print(
            f"RESUME_ATTACHED episode={self.episode} step={self.tick}",
            flush=True
        )
