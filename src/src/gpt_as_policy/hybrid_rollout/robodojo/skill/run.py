...
def turn_content_items(packet, *, images, max_image_edge=DEFAULT_CODEX_IMAGE_MAX_EDGE):
    """Convert dynamic-tool output content into turn/start input content."""
    items = content_items(packet, images=images, max_image_edge=max_image_edge)
    converted = []
    for item in items:
        if item["type"] == "inputText":
            converted.append({"type": "text", "text": item["text"]})
        elif item["type"] == "inputImage":
            converted.append({"type": "image", "url": item["imageUrl"]})
        else:
            raise ValueError(f"Unsupported bounded turn content type: {item['type']}")
    return converted
...
class CodexPolicy:
    def __init__(self, workspace, codex, *, timeout=900, transport_factory=StdioAppServer,
        ...
        self.bounded_context = os.environ.get('CODEX_BOUNDED_CONTEXT', '0') == '1'
        ...
        self.specs = tool_specs(self.method)
        specs = self.specs
    ...
    def _start_thread(self):
        response = self.transport.request('thread/start', dict(
            ...
            config=dict(model_reasoning_effort=EFFORT),
            developerInstructions=self.prompt,
            dynamicTools=self.specs,
            ephemeral=False,
            ...
            self.timeout)
        if response.get('model') != MODEL or response.get('reasoningEffort') != EFFORT:
            raise RuntimeError('Fresh Codex thread model/effort mismatch')

        self.thread_id = response["thread"]["id"]
        return self.thread_id

    def _bounded_turn_input(self, rollout):
        from ..prompt_context import task_context
        obs = rollout.observation_packet
        packet = dict(
            bounded_context_revision='v0.2.1',
            runtime_directive=(
                'Use only the rollout service tools. '
                'Do not use shell, filesystem, web, MCP, imageView, '
                'or exploratory tools. Make the next rollout decision '
                'directly from the supplied observation, RGB images, '
                'task context, proposal/FK, and same-episode evidence.'
            ),
            task=rollout.task,
            task_context=task_context(rollout.task),
            observation=obs,
            images=obs.get('images', []),
            next_call=rollout.next_call())
        return turn_content_items(packet, images=True,
                             max_image_edge=self.image_max_edge)

    def _turn(self, text, input_items=None):
        if input_items is None:
            input_items = [dict(type='text', text=text)]
        result = self.transport.request('turn/start', dict(threadId=self.thread_id,
            ...
            runtimeWorkspaceRoots=[str(self.agent_workspace)], input=input_items), self.timeout)
    ...
    def run(self, rollout):
        bounded_directive = (
            'BOUNDED CONTEXT v0.2.1. Use only rollout service tools; '
            'do not use shell/files/web/MCP/exploration. '
            if self.bounded_context else ''
        )
        turn_id = self._turn(bounded_directive + 'Act as the autonomous policy agent for this single simulation rollout. '
        ...
        while True:
            ...
            if method == 'thread/tokenUsage/updated' and params.get('threadId') == self.thread_id:
                ...
                self.transport.reply(event['id'], dict(success=success,
                    contentItems=content_items(packet, images=success and name != 'pi05_infer' and not (
                        self.bounded_context and
                        name == 'robodojo_execute' and
                        rollout.phase != 'done'),
                                               max_image_edge=self.image_max_edge)))
                if (success and self.bounded_context
                        and name == 'robodojo_execute'
                        and rollout.phase != 'done'):
                    old_thread = self.thread_id
                    old_turn = turn_id
                    try:
                        self.transport.request(
                            'turn/interrupt',
                            dict(threadId=old_thread, turnId=old_turn),
                            min(30, self.timeout))
                    except Exception:
                        pass
                    self._start_thread()
                    turn_id = self._turn(
                        '',
                        input_items=self._bounded_turn_input(rollout))
                    continuations = 0
                    network_error = None
                    network_consecutive = 0
                    print(json.dumps(dict(
                        event='bounded_context_rotate',
                        old_thread_id=old_thread,
                        thread_id=self.thread_id,
                        step_id=rollout.tick)), flush=True)
...
def main():
    ...
    args.output.mkdir(parents=True, exist_ok=args.resume_from is not None)
    ...
    try:
        ...
        if args.resume_from is None:
            worker_workspace = args.output / 'codex_workspace'
        else:
            resume_idx = 1
            while True:
                candidate = args.output / f'codex_workspace_resume_{resume_idx:02d}'
                if not candidate.exists():
                    worker_workspace = candidate
                    break
                resume_idx += 1
        worker = CodexPolicy(worker_workspace, args.codex, method=args.evaluation_method)
        ...
        if args.resume_from is not None:
            if args.evaluation_method != 'pi05_plus_gpt':
                raise ValueError('--resume-from requires pi05_plus_gpt')
            rollout.resume(args.resume_from)
        worker.run(rollout)
    except BaseException:
        error_text = traceback.format_exc()
        write_json(args.output/'failure.json', dict(error=error_text,
            controller_version=CONTROLLER_VERSION,
        ...
        preserve_live = 'usageLimitExceeded' in error_text
        if preserve_live:
            print(json.dumps(dict(event='live_episode_preserved', step_id=rollout.tick if rollout else 0)), flush=True)
        elif rollout is not None and rollout.episode is not None and rollout.phase != 'done':
            try:
